...
while True:
	...
	# create a dict to store all objects
	objects = {}
	...
	for i in np.arange(0, predictions.shape[2]):
		...
		if confidence > args["confidence"]:
			...
			# Update the objects dictionary with the key as the label and the value as count
			# If the label is already present in the dictionary, increment the count
			# Else, add the label to the dictionary and set the count to 1
			objects[CLASSES[idx]] = objects.get(CLASSES[idx], 0) + 1
      # Here if object is decteced then object number is increased based on detection of the object
	...
	# loop over the objects dictionary
	text = ""
	for (k, v) in objects.items():
		# draw the label and associated probability to the output frame
		text +="{}: {}".format(k, v)
	cv2.putText(frame, text, (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)

			# cv2.putText(frame, "Hello", (10,10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, COLORS[idx], 2)

	# show the output frame
	cv2.imshow("Frame", frame)
	# print("Objects detected in current frame: ")
	# print(objects)
